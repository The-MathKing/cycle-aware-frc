...
\noindent \textbf{A note on rigor.} The two results in this section (Heuristic Claims~\ref{thm:spectralgap} and \ref{thm:cheeger}) are \emph{not} proved to the standard of the rest of the paper: the derivation below relies on an unproven approximation step (\cref{eq:heuristic_approx}) with no error term. We therefore use a distinct "Heuristic Claim" environment, rather than Proposition/Theorem, and give a "Heuristic derivation" rather than a \texttt{proof} block, so the typography does not overstate what has actually been established. See the Limitations section for further discussion.

\begin{heuristicclaim}[Heuristic Bipartite Spectral Gap Lower Bound]
...
\begin{heuristicclaim}[Heuristic Bipartite Cheeger Inequality]
\label{thm:cheeger}
...
\label{alg:spspmm}
\SetAlgoLined
\KwIn{Sparse Bipartite Adjacency Matrix $A$, Node set $V$}
...
The core concept of utilizing 4-cycles to circumvent bipartite collapse is a highly intuitive, incremental progression in network science. In particular, our formulation explicitly builds upon the Augmented Forman 4-cycle (AF4) structure previously established by Fesser et al. \cite{fesser2024augmentations}, extending it to signed networks, bounding it relative to unweighted degrees, and formulating it computationally via SpSpMM.
