...
Evaluating $F^*$ via the Masked Sparse-Sparse Matrix Multiplication (SpSpMM) bypasses the catastrophic memory overhead of naive sparse matrix cubing. The operator computes $(A \cdot B) \circ S$, where $\circ$ is the Hadamard product and $S$ dictates the sparsity pattern across CSR vectors.
...
\begin{itemize}
    ...
    \item \textbf{LightGCN (Graph Neural Network):} A state-of-the-art bipartite message-passing architecture \cite{he2020lightgcn}. We trained a 64-dimensional PyTorch Geometric implementation using BPR loss across the identically masked train/test splits. 
\end{itemize}
