...
\begin{proof}
By the Kantarovich dual formulation, the Wasserstein distance for Ollivier-Ricci curvature involves shifting mass between the neighborhood distributions $m_u$ and $m_v$. In a bipartite setting, mass at $u' \in N(v)\setminus\{u\}$ must be transported to $v' \in N(u)\setminus\{v\}$. The minimum transport cost distance $d(u', v')$ is strictly $1$ if and only if the edge $(u', v')$ exists, which identically forms a 4-cycle $(u, v, u', v')$. Therefore, minimizing the $W_1$ transport cost is structurally isomorphic to maximizing the 4-cycle density between $N(u)$ and $N(v)$.

By explicitly extracting the bounding 4-cycles $\Sigma_{C_4}(e)$ and scaling by the theoretical maximum, $F^*(e)$ evaluates the exact fraction of mass that can be transported at minimal distance $1$. Consequently, $F^*(e)$ bounds the Ollivier-Ricci transport cost from below, replacing an $\mathcal{O}(d^3_{max})$ linear program with a deterministic $\mathcal{O}(d_{max})$ cycle enumeration.
\end{proof}
...
\begin{proof}
Because $A^2$ is strictly symmetric, the column extraction $(A^2)_{:,v}$ is mathematically identical to the transposed row extraction $((A^2)_{v,:})^T$. In CSR format, row extraction is an $\mathcal{O}(1)$ pointer operation, whereas column extraction requires an $\mathcal{O}(N)$ memory scan. By evaluating $A_{u,:} \cdot ((A^2)_{v,:})^T$, the SDDMM algorithm avoids instantiating the dense matrix $A^3$. Instead, it computes the inner product of two sparse CSR rows. This guarantees $\mathcal{O}(|E| d_{max})$ time complexity while ensuring memory accesses remain contiguous, drastically mitigating L3 cache misses in scale-free topologies with extreme degree variance.
\end{proof}
...
As shown in Table \ref{tab:clustering}, applying standard spectral clustering on the unweighted adjacency matrix fails under high stochastic noise (ARI plunges to 0.231). Conversely, substituting the adjacency matrix with the $F^*$-weighted matrix $W_{u,v} = \max(0, F^*(u,v))$ isolates the strictly positive geometry. The geometrically-weighted spectral clustering sustains an ARI of 0.785 even under extreme structural entropy, confirming that $F^*$ mathematically isolates genuine bipartite communities.
